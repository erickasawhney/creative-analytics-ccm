...
class _HealthHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format, *args):
        # Keep logs quiet in Streamlit reruns.
        return
...
def _start_health_server():
    ...
    def _run_server():
        try:
            ...
            # In Streamlit reruns, the server may already be running.
            pass
...
if uploaded_file is not None:
    ...
    if st.session_state.get('edited_creative_names'):
        ...
        if filtered["Edited_Group_Key"].duplicated().any():
            ...
            if video_started_col and video_completed_col and video_started_col in filtered.columns and video_completed_col in filtered.columns:
    ...
    chart_df["Label"] = chart_df["Display_Name"].apply(lambda s: _wrap_into_html(s, width=25))
...
*Note: Analysis based on creative naming patterns and visual assessment. For deeper insights, consider professional creative testing.*
    ...
    if st.session_state.edited_creative_names:
        ...
        if table_df["Edited_Identifier"].duplicated().any():
            ...
            # Recalculate VCR if video columns exist
            if video_started_col and video_completed_col and video_started_col in table_df.columns and video_completed_col in table_df.columns:
